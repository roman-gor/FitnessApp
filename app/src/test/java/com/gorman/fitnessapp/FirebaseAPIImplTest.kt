...
class FirebaseAPIImplTest {
    ...
    private lateinit var firebaseAPI: PostgreSQLServiceImpl
    ...
    fun setUp() {
        ...
        firebaseAPI = PostgreSQLServiceImpl(database)
    }
    ...
    fun `insertMealPlan should write template and items to correct Firebase paths`() = runTest {
        // Arrange
        val template =
            MealPlanTemplateSupabase(userId = 123, name = "Test Template", description = "Desc")
        val items = listOf(
            MealPlanItemSupabase(
                mealId = 1,
                mealType = "Breakfast",
                templateId = 123,
                date = 17400000
            ),
            MealPlanItemSupabase(mealId = 2, mealType = "Lunch", templateId = 123, date = 17400000)
        )
        ...
        val templateCaptor = argumentCaptor<MealPlanTemplateSupabase>()
    ...
    fun `insertMealPlan with empty items should only write template`() = runTest {
        ...
        val template =
            MealPlanTemplateSupabase(userId = 123, name = "Test Template", description = "Desc")
        val items = emptyList<MealPlanItemSupabase>()
