...
class GetExercisesUseCaseTest {
    ...
    @Before
    fun setUp() {
        getExercisesUseCase = GetExercisesUseCase(databaseRepository, firebaseRepository)
    }
    ...
    fun `invoke() with non-empty cache should return cached data and not fetch from remote`() = runTest {
        ...
        verify(firebaseRepository, never()).getExercises()
    ...
    fun `invoke() with empty cache should fetch from remote, save and return`() = runTest {
        ...
        whenever(firebaseRepository.getExercises()).thenReturn(remoteExercises)
